...
dataPathExtra = rootPath + '/synthesis/'
sDatPathExtra = dataPathExtra + '/security_patch/'
pDatPathExtra = dataPathExtra + '/positives/'
nDatPathExtra = dataPathExtra + '/negatives/'
...
def demoTwinExtra():
    ...
    if (not os.path.exists(tempPath + '/props.npy')):
        ...
        print('[INFO] <GetDiffProps> Load ' + str(len(diffProps)) + ' diff property data from ' + tempPath + '/props.npy.')
    # load data.
    if (not os.path.exists(tempPath + '/dataExtra.npy')):  # | (not _DEBUG_)
        dataLoadedExtra = ReadDataExtra()
    else:
        dataLoadedExtra = np.load(tempPath + '/dataExtra.npy', allow_pickle=True)
        print('[INFO] <ReadDataExtra> Load ' + str(len(dataLoadedExtra)) + ' raw data from ' + tempPath + '/dataExtra.npy.')
    # get the diff file properties.
    if (not os.path.exists(tempPath + '/propsExtra.npy')):
        diffPropsExtra = GetDiffPropsExtra(dataLoadedExtra)
    else:
        diffPropsExtra = np.load(tempPath + '/propsExtra.npy', allow_pickle=True)
        print('[INFO] <GetDiffPropsExtra> Load ' + str(len(diffPropsExtra)) + ' diff property data from ' + tempPath + '/propsExtra.npy.')

    # process the tokens for original and extra data.
    diffProps = ProcessTokens(diffProps, dType=0, cType=_CTYP_)
    diffProps = AbstractTokens(diffProps, iType=_NIND_, lType=_NLIT_)
    diffPropsExtra = ProcessTokens(diffPropsExtra, dType=0, cType=_CTYP_)
    diffPropsExtra = AbstractTokens(diffPropsExtra, iType=_NIND_, lType=_NLIT_)
    # get the dictionary and embedding matrix for diff code.
    combineProps = CombineList(diffProps, diffPropsExtra)
    diffVocab, _ = GetDiffVocab(combineProps)
    diffDict = GetDiffDict(diffVocab)
    twinPreWeights = GetDiffEmbed(diffDict, _TwinEmbedDim_)
    # get mappings for diff code.
    twinProps, _ = DivideBeforeAfter(diffProps)
    twinData, twinLabels = GetTwinMapping(twinProps, _TwinMaxLen_, diffDict)
    twinData = UpdateTwinTokenTypes(twinData)
    twinPropsExtra, _ = DivideBeforeAfter(diffPropsExtra)
    twinDataExtra, twinLabelsExtra = GetTwinMapping(twinPropsExtra, _TwinMaxLen_, diffDict)
    twinDataExtra = UpdateTwinTokenTypes(twinDataExtra)
    # ------------------------------------------------------------------------------
    # get the commit messages from data.
    if (not os.path.exists(tempPath + '/msgs.npy')):
        commitMsgs = GetCommitMsgs(dataLoaded)
        ...
        commitMsgs = np.load(tempPath + '/msgs.npy', allow_pickle=True)
        print('[INFO] <GetCommitMsg> Load ' + str(len(commitMsgs)) + ' commit messages from ' + tempPath + '/msgs.npy.')
    if (not os.path.exists(tempPath + '/msgsExtra.npy')):
        commitMsgsExtra = GetCommitMsgsExtra(dataLoadedExtra)
    else:
        commitMsgsExtra = np.load(tempPath + '/msgsExtra.npy', allow_pickle=True)
        print('[INFO] <GetCommitMsgExtra> Load ' + str(len(commitMsgsExtra)) + ' commit messages from ' + tempPath + '/msgsExtra.npy.')

    # get the dictionary and embedding matrix for message.
    combineMsgs = CombineList(commitMsgs, commitMsgsExtra)
    msgVocab, _ = GetMsgVocab(combineMsgs)
    msgDict = GetMsgDict(msgVocab)
    msgPreWeights = GetMsgEmbed(msgDict, _MsgEmbedDim_)
    # get mappings for message.
    msgData, msgLabels = GetMsgMapping(commitMsgs, _MsgMaxLen_, msgDict)
    msgDataExtra, msgLabelsExtra = GetMsgMapping(commitMsgsExtra, _MsgMaxLen_, msgDict)
    # ------------------------------------------------------------------------------
    # combine the twin data with the message data.
    data, label = CombineTwinMsgs(twinData, msgData, twinLabels, msgLabels)
    dataExtra, labelExtra = CombineTwinMsgs(twinDataExtra, msgDataExtra, twinLabelsExtra, msgLabelsExtra)
    # split data into rest/test dataset.
    dataTrain, labelTrain, dataTest, labelTest = SplitData(data, label, 'test', rate=0.2)
    print('[INFO] <demoTwin> Get ' + str(len(dataTrain)) + ' TRAIN data, ' + str(len(dataTest))
          + ' TEST data. (Total: ' + str(len(dataTrain) + len(dataTest)) + ')')
    dataTrain, labelTrain, dataTest, labelTest = AppendDataset(dataTrain, labelTrain, dataTest, labelTest, dataExtra, labelExtra, rate=0.2)
...
def AppendDataset(dataTrain, labelTrain, dataTest, labelTest, dataExtra, labelExtra, rate=0.2):

    # convert numpy to list
    dataTrain = dataTrain.tolist()
    labelTrain = labelTrain.tolist()
    dataTest = dataTest.tolist()
    labelTest = labelTest.tolist()

    # get the original patch list.
    fileList = []
    for line in open(tempPath + 'filelist.txt', 'r'):
        path, file = os.path.split(line[:-1])
        fileList.append(file)
    # get the extra patch list.
    fileListExtra = []
    for line in open(tempPath + 'filelistExtra.txt', 'r'):
        path, file = os.path.split(line[:-1])
        fileListExtra.append(file[:-6])

    # get the original index.
    dataList = np.load(tempPath + '/split_TEST.npy')
    num = math.floor(len(dataList) * rate)
    # get the training and testing list.
    testList = [fileList[dataList[n]] for n in range(num)]
    trainList = [fileList[dataList[n]] for n in range(num, len(dataList))]

    # append the extra data.
    for i in range(len(fileListExtra)):
        if fileListExtra[i] in trainList:
            dataTrain.append(dataExtra[i])
            labelTrain.append(labelExtra[i])
        elif fileListExtra[i] in testList:
            dataTest.append(dataExtra[i])
            labelTest.append(labelExtra[i])
        else:
            print('[ERROR] <AppendDataset> ' + fileListExtra[i] + 'is not in original dataset!')

    print('[INFO] <AppendDataset> Get ' + str(len(dataTrain)) + ' TRAIN data, ' + str(len(dataTest))
          + ' TEST data. (Total: ' + str(len(dataTrain) + len(dataTest)) + ')')

    return np.array(dataTrain), np.array(labelTrain), np.array(dataTest), np.array(labelTest)
...
def ReadDataExtra():
    ...
    if not os.path.exists(tempPath):
        os.mkdir(tempPath)
    fp = open(tempPath + 'filelistExtra.txt', 'w')

    # initialize data.
    data = []
    # read security patch data.
    for root, ds, fs in os.walk(sDatPathExtra):
        for file in fs:
            filename = os.path.join(root, file).replace('\\', '/')
            fp.write(filename + '\n')
            commitMsg = ReadCommitMsg(filename)
    ...
    # read positive data.
    for root, ds, fs in os.walk(pDatPathExtra):
        for file in fs:
            filename = os.path.join(root, file).replace('\\', '/')
            fp.write(filename + '\n')
            commitMsg = ReadCommitMsg(filename)
    ...
    # read negative data.
    for root, ds, fs in os.walk(nDatPathExtra):
        for file in fs:
            filename = os.path.join(root, file).replace('\\', '/')
            fp.write(filename + '\n')
            commitMsg = ReadCommitMsg(filename)
    ...
    # save dataLoaded.
    if not os.path.exists(tempPath + '/dataExtra.npy'):
        np.save(tempPath + '/dataExtra.npy', data, allow_pickle=True)
        print('[INFO] <ReadDataExtra> Save ' + str(len(data)) + ' raw data to ' + tempPath + '/dataExtra.npy.')

    return data

def GetDiffPropsExtra(data):
    '''
    Get the properties of the code in diff files.
    :param data: [[[line, , ], [[line, , ], [line, , ], ...], 0/1], ...]
    :return: props - [[[tokens], [nums], [nums], 0/1], ...]
    ...
    if not os.path.exists(tempPath):
        os.mkdir(tempPath)
    if not os.path.exists(tempPath + '/propsExtra.npy'):
        np.save(tempPath + '/propsExtra.npy', props, allow_pickle=True)
        print('[INFO] <GetDiffPropsExtra> Save ' + str(len(props)) + ' diff property data to ' + tempPath + '/propsExtra.npy.')

    return props

def GetCommitMsgsExtra(data):
    '''
    Get the commit messages in diff files.
    :param data: [[[line, , ], [[line, , ], [line, , ], ...], 0/1], ...]
    :return: msgs - [[[tokens], 0/1], ...]
    ...
    if not os.path.exists(tempPath):
        os.mkdir(tempPath)
    if not os.path.exists(tempPath + '/msgsExtra.npy'):
        np.save(tempPath + '/msgsExtra.npy', msgs, allow_pickle=True)
        print('[INFO] <GetCommitMsgExtra> Save ' + str(len(msgs)) + ' commit messages to ' + tempPath + '/msgsExtra.npy.')
